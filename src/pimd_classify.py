...
#   v1.5 default corpus repointed out of the retired ML/ tree into src/data/corpora/
#   v1.4 FIX heatmap band order keyed on pulse_us, not the delays_us[0] proxy
#   v1.3 FIX heatmap rows sorted delay-descending (longest at top) instead
#        of ascending -- now matches the standard grid orientation
#   v1.2 configurable strip charts; Band-mean and Per-delay-normalized modes
#   v1.1 heatmap range/axes; band-mean pulse-width ticks; Event Log sorting fix
#   v1.0 initial release: Qt-free Engine (baseline/detect/features/classify), GUI, --headless replay
###############################################################################
...
# v1.5: was ../ML/V2/PIMD_target_corpus_signatures_v2.csv, which stopped existing
# when ML/ was retired. Repointed at the newest corpus actually on disk. Note this
# is a PREVIOUS-EPOCH corpus (cal_63_air_bat_v3, 63 cells) and the operating
# profile is now cal_3x10_v1 (30 cells), so the geometry guard will reject it
# against a live profile -- as it should. Pass --corpus once a 30-cell corpus
# exists; this default only spares the common replay case a flag.
DEFAULT_CORPUS = os.path.join(SCRIPT_DIR, 'data', 'corpora',
                              'gui_signatures_targets_v3_20260728_142316.csv')
...
def run_gui(args):
    ...
    class MainWindow(QMainWindow):
        def __init__(self, args):
            ...
            # Ascending PULSE WIDTH -- shortest-pulse band first -- matches this
            # project's standard grid orientation (pimd_rawlog.py header / CHANGELOG
            # v1.13): with invertY(True) on heat_plot, this puts the shortest pulse
            # at the top of the heatmap. Lowest top-left, highest bottom-right.
            # v1.4: the key was `delays_us[0]`, a proxy for pulse width that holds
            # for every calibrated profile (shorter pulse releases the clamp earlier)
            # but breaks when two bands are hand-given the same delay ladder --
            # cal_3x15_v1 starts both its 50 µs and 10 µs bands at 5.160 µs and came
            # out 50/10/100. Same fix as pimd_classviz.py v1.71; the two tools must
            # agree, or one profile renders two different ways.
            self._band_display_order = sorted(
                range(self._n_bands),
                key=lambda i: self.reference_profile['bands'][i]['pulse_us'])
...
def build_arg_parser():
    ...
    p.add_argument('--corpus', default=DEFAULT_CORPUS,
                    help='Corpus CSV for the identity classifier (default: the newest corpus in '
                         'src/data/corpora/, which is previous-epoch 63-cell).')
