...
- **Standard Operating Conditions (SoC)** *(established 2026-06-18)*: Mode 1 · 10.0 kHz /
  ...
  take noise-floor readings before this point). Reference capture: `References/images/GUI-steady-state-256-1024.jpg`.
...
| `References/images/GUI-steady-state-256-1024.jpg` | SoC steady-state reference capture — settled noise floor and thermal drift; Mode 1 at SoC conditions, first half DS 256 / second half DS 1024 |
| `References/images/GUI-noise-comp.jpg` | GUI noise comparison — DS 256 vs DS 1024 side-by-side, Mode 1 at SoC conditions |
| `References/images/early-discrimination-tests.JPEG` | Early discrimination test captures |
| `References/images/pcb-coil-baseline.JPEG` | Bench baseline setup, pre-enclosure — main board mounted on the v4 concentric TX/RX coil (epoxied to Perspex), battery supply via 18 V pack adapter |
| `References/images/warmup-with-8ns-steps.jpg` | Mode 1 GUI capture during warm-up — sample delay stepped in 8 ns grid increments at 25 kHz / 10.4 µs / DS 256; each grid step lands ≈ 5 mV apart on the steep decay, with warm-up drift visible as the slope between steps. Illustrates why calibrated delays snap to the 8 ns PWM grid |
| `References/images/new-training-data.jpg` | classviz v1.32 Analysis tab — first capture session under the structured target-metadata regime (cal_63_air_v1 loaded via Load & Run, registry-backed target combo + placement fields, Std Dev heatmap mode, per-pulse-width and per-delay profile charts) |
| `References/images/training-targets-v3.JPEG` | Target set v3 — the physical objects behind `src/data/targets/targets_v1.csv` laid out on the bench (pipes, brass block, solder roll, copper crimps, gear, ferrite ring, silver items, spanner, shackle, plates, …) |
| `References/images/training-results-v1a.jpg` | **Previous-epoch** (cal_72_air_v2 corpus): normalised 5 cm band responses grouping 17 targets into ferrous-rising / crossover (SS pipe, lead pipe) / non-ferrous families — the τ-fingerprint result behind §13's discrimination claims; findings historical since the epoch reset |
...
Reference capture: `References/images/GUI-steady-state-256-1024.jpg` — first half of plot at DS 256,
