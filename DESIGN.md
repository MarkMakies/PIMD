...
| `README.md` | Repository front page — project summary and entry point for a first-time reader. Also carries the **Toolchain & workflow** pipeline diagram and workflow ordering, inherited from the retired `USAGE.md` (2026-08-13); per-app intent and operation now live in each tool's own file header. |
...
| 17.14 | **Thermal and supply drift separate by band-magnitude ratio, not by sign** *(corrected 2026-08-12)*. Bulk warm-up moves band 0 **~16× further** than band 1 (−7.6 % vs −0.46 %) and in the **same** direction; a supply shift moves both together within ~1.5× (−0.81 % vs −0.54 % over 2.1 V). Only an **airflow** change splits them by sign (band 0 +3.7 %, band 1 −0.2 %). The earlier opposite-directions sign test (r = +0.99, 7-band epoch) therefore identifies a **cooling change**, not thermal drift in general — and a warm-up would be misread as a supply shift by it. | current |
