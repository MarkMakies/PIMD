...
| `src/pimd_corpus_check.py` | Corpus-level acceptance checker (**v1.4**, offline CLI) — shape distance-invariance, split-half SNR, canary drift, repeat consistency (within- and cross-session), falloff fit, optional cross-campaign baseline. Re-tracked 2026-07-15; v1.4 loudly rejects the new `target_id` schema — re-homing its checks onto the new columns is a planned follow-up |
...
| `References/training-targets-v3.JPEG` | Target set v3 — the physical objects behind `src/data/training_lists/targets.csv` laid out on the bench (pipes, brass block, solder roll, copper crimps, gear, ferrite ring, silver items, spanner, shackle, plates, …) |
